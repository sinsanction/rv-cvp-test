...
void bench_pool1_prepare() {
  bench_srand(1);
  A = RandomInitImage_SC(w, h, 16, 1);
  test_pass = 1;
}

void bench_pool1_run() {
  B = MaxPooling_SC(A, k, strides);
  C = StdIns_MaxPooling_SC(A, k, strides);
}

int bench_pool1_validate() {
